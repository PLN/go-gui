...
- Direct go-gui consumers (require a go-gui bump on release): **go-charts,
  go-edit, go-kite, go-map, go-term**. Verified against each repo's `go.mod`
  `require` (all `github.com/go-gui-org/*`). go-glyph is _upstream_ — go-gui
  depends on it, not the reverse; never a bump target.
